...
public class OrderEventService(LogisticDataContext _datContext, OrderHubService _hubService)
{
    public async Task<OrderEvent> RegisterEventAsync(CreateOrderEventDTO dto)
    {
        var order = await _datContext.Orders.FirstOrDefaultAsync(o => o.Id == dto.OrderId)
            ?? throw new KeyNotFoundException($"Pedido {dto.OrderId} no encontrado.");

        var newStatus = ResolveStatus(dto.EventType, order.OrderStatus);

        int attempt = dto.EventType is OrderEventType.PickedUpByDriver or OrderEventType.OutForDelivery
                        ? await CountDeliveryAttempts(dto.OrderId) + 1
                        : await CountDeliveryAttempts(dto.OrderId);

        var ev = OrderEvent.Create(
            orderId: dto.OrderId,
            eventType: dto.EventType,
            statusAfter: newStatus,
            userID: dto.ActorUserID,
            notes: dto.Notes,
            baglokLocation: dto.BaglokLocation!,
            rescheduledFor: dto.RescheduledFor,
            deliveryAttempt: attempt
        );

        order.UpdateStatus(newStatus);
        _datContext.OrderEvents.Add(ev);
        await _datContext.SaveChangesAsync();

        // ── Notificar en tiempo real via SignalR ──────────────────
        var eventDto = MapToDTO(ev);
        await _hubService.NotifyNewEventAsync(dto.OrderId, eventDto);
        await _hubService.NotifyStatusChangedAsync(dto.OrderId, order.TrackingCode, newStatus.ToString(), StatusColor(newStatus));
    ...
    public Task<OrderEvent> RegisterReceivedAsync(int orderId, string operatorId, string? notes = null)
        => RegisterEventAsync(new CreateOrderEventDTO
        {
            OrderId = orderId,
            EventType = OrderEventType.RegisteredInWarehouse,
            ActorUserID = operatorId,
            Notes = notes,
        });

    public Task<OrderEvent> RegisterPickupAsync(int orderId, string driverId)
        => RegisterEventAsync(new CreateOrderEventDTO
        {
            OrderId = orderId,
            EventType = OrderEventType.PickedUpByDriver,
            ActorUserID = driverId,
        });

    public Task<OrderEvent> RegisterDeliveredAsync(int orderId, string driverId, string? notes = null)
        => RegisterEventAsync(new CreateOrderEventDTO
        {
            OrderId = orderId,
    ...
    public Task<OrderEvent> RegisterAbsentAsync(int orderId, string driverId, string? notes = null)
        => RegisterEventAsync(new CreateOrderEventDTO
        {
            OrderId = orderId,
            EventType = OrderEventType.RecipientAbsent,
            ActorUserID = driverId,
            Notes = notes,
        });

    public Task<OrderEvent> RegisterStoredInBaglokAsync(
        int orderId, string operatorId, string baglokLocation, string? notes = null)
        => RegisterEventAsync(new CreateOrderEventDTO
        {
            OrderId = orderId,
            EventType = OrderEventType.StoredInBaglok,
            ActorUserID = operatorId,
            BaglokLocation = baglokLocation,
            Notes = notes,
        });

    public Task<OrderEvent> RegisterRescheduledAsync(
        int orderId, string supervisorId, DateTime rescheduledFor, string? notes = null)
        => RegisterEventAsync(new CreateOrderEventDTO
        {
            OrderId = orderId,
            EventType = OrderEventType.Rescheduled,
            ActorUserID = supervisorId,
            RescheduledFor = rescheduledFor,
            Notes = notes,
        });

    public Task<OrderEvent> RegisterReleasedFromBaglokAsync(int orderId, string operatorId)
        => RegisterEventAsync(new CreateOrderEventDTO
        {
            OrderId = orderId,
            EventType = OrderEventType.ReleasedFromBaglok,
            ActorUserID = operatorId,
        });
